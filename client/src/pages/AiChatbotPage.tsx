  ...
  return (
    <div className="min-h-screen bg-[#0B0F12] text-[#F3F6F8] flex flex-col selection:bg-amber-500/30 selection:text-amber-200">
      ...
      <header className="sticky top-0 z-40 bg-[#11171D]/90 backdrop-blur-xl border-b border-white/10 px-4 sm:px-6 py-3">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/dashboard"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-none bg-white/[0.04] hover:bg-white/[0.08] text-xs font-mono text-stone-300 hover:text-stone-100 transition-all border border-white/10 shadow-sm"
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-none bg-gradient-to-br from-amber-500/25 to-amber-600/10 border border-amber-500/40 flex items-center justify-center text-amber-400 shadow-md shrink-0">
                <Sparkles size={17} />
              </div>
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-[9px] font-mono font-semibold px-2 py-0.5 rounded-none bg-amber-500/15 text-amber-300 border border-amber-500/30">
                    GEMINI 3.5 FLASH
                  </span>
                </div>
                <p className="text-[10.5px] text-stone-400 hidden sm:block font-mono">
                  {t("Geotechnical Decision Support, IMD Grounding & Evacuation Navigation")}
                </p>
              </div>
            </div>
          </div>
            ...
            {authMeQuery.data?.user ? (
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-none bg-emerald-950/40 hover:bg-emerald-900/50 border border-emerald-500/40 text-xs font-medium text-emerald-300 transition-colors shadow-sm"
                ...
                {quota?.isUnlimited ? (
                  <span className="px-2 py-1 rounded-none bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[10px] font-mono font-bold">
                    🛡️ {t("UNLIMITED")}
                  </span>
                ) : (
                  <span
                    className="px-2.5 py-1 rounded-none bg-amber-500/15 text-amber-300 border border-amber-500/30 text-[10px] font-mono font-bold"
                    title={`Daily limit: ${quota?.limit || 30} queries. Resets in ${quota?.resetsInHours || 24}h`}
                  >
                    ⚡ {quota?.remaining ?? 30}/{quota?.limit ?? 30} {t("QUOTA")}
                  </span>
                )}
              </div>
            ) : (
              <button
                type="button"
                onClick={() => setGoogleAuthModalOpen(true)}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-none bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-stone-950 text-xs font-bold transition-all shadow-lg shadow-amber-500/20 active:scale-95"
              >
                <Sparkles size={13} />
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none">
            <span className="text-[11px] font-mono text-stone-400 uppercase tracking-wider mr-1 shrink-0 flex items-center gap-1 font-semibold">
              <MapPin size={12} className="text-amber-400" /> {t("ACTIVE ZONE")}:
            </span>
            {GLOBAL_GEOTECHNICAL_STATIONS.map((z) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 rounded-none text-xs font-medium shrink-0 transition-all flex items-center gap-1.5 ${
                    isSelected
          ...
          <div className="flex items-center gap-3 text-xs font-mono text-stone-300 shrink-0 bg-[#162028] px-3 py-1.5 rounded-none border border-white/5 shadow-inner">
            <div className="flex items-center gap-1.5" title="Monitored 24h Cumulative Rainfall">
              <CloudRain size={13} className="text-blue-400" />
              <span>{t("RAIN")}: <b className="text-white">{currentZone.rainfall}mm</b></span>
            </div>
            ...
            <div className="flex items-center gap-1.5" title="Volumetric Water Content Soil Saturation">
              ...
              <span>{t("SOIL")}: <b className="text-white">{currentZone.soil}%</b></span>
            </div>
            <div className="h-3 w-px bg-stone-700" />
            <div className="flex items-center gap-1.5" title="Biaxial Inclinometer Slope Displacement">
              ...
              <span>{t("TILT")}: <b className="text-white">{currentZone.tilt}°</b></span>
            </div>
            <div className="h-3 w-px bg-stone-700" />
            <div className="flex items-center gap-1.5 font-bold" style={{ color: currentZone.tier === "CRITICAL" ? "#EF4444" : currentZone.tier === "WATCH" ? "#F59E0B" : "#10B981" }}>
              <ShieldAlert size={13} />
              <span>{t(currentZone.tier)} ({currentZone.riskScore}%)</span>
            </div>
            <div className="h-3 w-px bg-stone-700" />
            <button
              type="button"
              ...
              className="px-2 py-0.5 rounded-none bg-red-500/20 hover:bg-red-500/30 text-red-300 text-[10px] font-mono border border-red-500/30 transition-colors"
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-5 flex-1 flex flex-col w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5 mb-4 p-1.5 rounded-none bg-[#11171D] border border-white/10 shadow-xl">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0 scrollbar-none whitespace-nowrap">
            <button
              type="button"
              onClick={() => setActiveTab("CHATBOT")}
              className={`px-3.5 py-2 rounded-none text-xs font-bold flex items-center gap-2 shrink-0 transition-all ${
                activeTab === "CHATBOT"
            ...
            >
              ...
              <span className="text-[9px] font-mono px-1.5 py-0.5 bg-black/25 rounded-none">MULTI-TURN</span>
            </button>
            ...
            >
              <Search size={14} />
              <span>{t("SEARCH GROUNDING (IMD)")}</span>
              <span className="text-[9px] font-mono px-1.5 py-0.5 bg-black/25 rounded-none">CITATIONS</span>
            </button>
            ...
            >
              <MapPin size={14} />
              <span>{t("MAPS GROUNDING (PASSES)")}</span>
              <span className="text-[9px] font-mono px-1.5 py-0.5 bg-black/25 rounded-none">CORRIDORS</span>
            </button>
            ...
            >
              <Activity size={14} />
              <span>{t("SLOPE STABILITY")}</span>
            </button>
          </div>

          <div className="flex items-center gap-2 text-[10.5px] font-mono text-stone-400 px-2 shrink-0">
            <span>{t("LOCATION")}: <b className="text-stone-200">{currentZone.name}</b></span>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          {activeTab === "RISK_SYNTHESIS" && (
            <div className="p-6 rounded-none bg-[#11171D] border border-white/10 text-stone-200 space-y-6 shadow-xl">
              <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <div>
                  <span className="text-xs font-mono text-amber-400 uppercase tracking-wider">
                    {t("SLOPE STABILITY BREAKDOWN")} · {currentZone.name}
                  </span>
                  <h2 className="text-xl font-bold text-white mt-1">
                    {t("What the sensors are seeing right now")}
                  </h2>
                </div>
                <div
                  className="px-4 py-2 rounded-none font-bold font-mono text-sm border flex items-center gap-2"
                  style={{
                ...
                >
                  <ShieldAlert size={16} />
                  <span>{t(currentZone.tier)} {t("RISK")} · {currentZone.riskScore}%</span>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 rounded-none bg-[#162028] border border-white/5">
                  <span className="text-xs font-mono text-stone-400">{t("24H RAINFALL")}</span>
                  <div className="text-2xl font-bold text-blue-400 mt-1">{currentZone.rainfall} mm</div>
                  <p className="text-xs text-stone-400 mt-1">
                    {t("Threshold: 100mm. Heavy continuous rain is loading water weight onto the hillside.")}
                  </p>
                </div>

                <div className="p-4 rounded-none bg-[#162028] border border-white/5">
                  <span className="text-xs font-mono text-stone-400">{t("SOIL MOISTURE")}</span>
                  <div className="text-2xl font-bold text-amber-400 mt-1">{currentZone.soil}% {t("SATURATED")}</div>
                  <p className="text-xs text-stone-400 mt-1">
                    {t("Capacitive probe indicates the soil matrix is nearing fluid saturation.")}
                  </p>
                </div>

                <div className="p-4 rounded-none bg-[#162028] border border-white/5">
                  <span className="text-xs font-mono text-stone-400">{t("SLOPE ANGLE DRIFT")}</span>
                  <div className="text-2xl font-bold text-purple-400 mt-1">{currentZone.tilt}° {t("TILT")}</div>
                  <p className="text-xs text-stone-400 mt-1">
                    {t("Biaxial tiltmeter shows continuous micro-shift along the bedrock contact plane.")}
                  </p>
                </div>
              </div>

              <div className="p-4 rounded-none bg-amber-500/10 border border-amber-500/30 text-xs text-amber-200">
                <div className="flex items-center gap-2 font-bold mb-1 text-amber-300">
                  <Sparkles size={14} /> {t("FIELD ADVISORY FOR PATROLS & DISTRICT CONTROL")}
                </div>
                <p className="leading-relaxed text-stone-300">
                  {t("Notify taluk revenue officers immediately. Stop heavy commercial trucks from entering the pass road. Send local emergency teams to check downstream homes and prepare high-ground school buildings for sheltering.")}
                </p>
              </div>
            </div>
          )}
      ...
      {/* Toast Notice */}
      {notice && (
        <div className="fixed bottom-4 right-4 z-50 px-4 py-2.5 rounded-none bg-stone-900 border border-stone-700 text-stone-100 text-xs shadow-2xl flex items-center gap-2 animate-fade-in">
